#!/usr/bin/env node
/**
 * Reads all .md files from /content/news/, parses frontmatter,
 * and generates the posts array for /src/data/news.ts
 */
...
const outFile = path.join(__dirname, '..', 'src', 'data', 'news.ts');
...
function parseFrontmatter(content) {
  ...
  while (i < lines.length) {
    const line = lines[i];
    // Skip array items and keys we don't need
...
// Only one post should be featured — pick the most recent one
...
// Also include the existing first article (already in news.ts) — skip if slug matches
const existingSlugs = new Set([
  'ai-sales-agents-replacing-sdrs-2026',
]);

// Keep existing article at top, plus filter out duplicates
...
// First, add the existing hero article
output += `  {
...
// Now add all new posts
for (const post of newPosts) {
  const featuredStr = post.featured ? '\n    featured: true,' : '';
  output += `  {
    slug: "${post.slug}",
    ...
    date: "${post.date}",
    readTime: "${post.readTime}",${featuredStr}
    body: \`${escapeForTemplateLiteral(post.body)}\`,
  },
`;
}
...
console.log(`Generated ${newPosts.length + 1} posts (1 existing + ${newPosts.length} new) in news.ts`);
console.log('Posts by date:');
for (const p of [{ slug: 'ai-sales-agents-replacing-sdrs-2026', date: '2026-03-18', featured: false }, ...newPosts]) {
  console.log(`  ${p.date} | ${p.featured ? 'FEATURED' : '        '} | ${p.slug}`);
}
