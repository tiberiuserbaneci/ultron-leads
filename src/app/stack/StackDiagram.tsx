...
export default function StackDiagram() {
  ...
  return (
    <div className="space-y-2">
          ...
          <div key={node.id}>
            <button
              ...
              className={`w-full text-left p-4 sm:p-5 rounded-xl border transition-all duration-200 ${
                isExpanded ? `${node.nodeBg} ${node.nodeBorder}` : "bg-[#111] border-[#222] hover:border-[#404040]"
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-[#999] text-xs sm:text-sm">{node.sublabel}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="2"
                    className={`text-[#999] transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}
                  >
                    <path d="M6 9l6 6 6-6"/>
                  </svg>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-[#1a1a1a] grid sm:grid-cols-2 gap-4 text-left" onClick={(e) => e.stopPropagation()}>
                    ...
                    <ul className="space-y-1.5">
                      {node.agentActions.map((action, j) => (
                        <li key={j} className="flex items-start gap-2 text-sm text-[#999]">
                          <span className="text-[#DA4E24] flex-shrink-0 mt-0.5">→</span>
                          {action}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="space-y-3">
                    <div className="bg-black rounded-xl p-4">
                      <div className="text-xs text-[#999] mb-1.5 font-mono">EXAMPLE OUTPUT</div>
                      <p className="text-sm text-white italic">{node.example}</p>
                    </div>
                    <div className="bg-[#100808] border border-red-900/20 rounded-xl p-4">
                      <div className="text-xs text-[#999]/70 mb-1.5 font-mono">WITHOUT THIS NODE</div>
                      <p className="text-sm text-[#999]">{node.withoutCost}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            {!isLast && (
              <div className="flex justify-center py-1">
                <div className="flex flex-col items-center gap-0.5">
                  ...
                  <svg width="12" height="8" viewBox="0 0 12 8" fill="none" className="text-[#555]">
      ...
      {/* Loop back arrow */}
      <div className="mt-4 flex items-center gap-3 text-xs text-[#666] font-mono p-3 border border-[#1a1a1a] rounded-xl">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#DA4E24]/50 flex-shrink-0">
