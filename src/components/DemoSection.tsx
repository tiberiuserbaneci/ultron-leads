...
/* ─── Chat Simulation (used only on /demo standalone) ─── */
...
/* ─── Live Stats Bar (exported for use in landing page) ─── */
...
/* ─── Demo Content (the interactive demo, no hero/header) ─── */
export function DemoContent({
  ...
  useEffect(() => {
    const slug = initialSlug || searchParams.get("prompt");
    if (slug) {
  ...
  return (
    <div className={embedded ? "" : "max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16"}>
      ...
      {/* Chat Input — only on standalone /demo */}
      {!embedded && (
        <div
          ref={chatRef}
      ...
      {/* Embedded chat ref for scroll target */}
      {embedded && <div ref={chatRef} />}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {filteredPrompts.map((p) => (
              <PromptCard
                ...
                onClick={() => {
                  if (embedded) {
                    // In embedded mode, skip the chat animation — go straight to workflow
                    setSelectedPrompt(p);
                    setWorkflowStarted(true);
                    haptic(20);
                    setTimeout(() => {
